...
uses
	Classes, SysUtils, Generics.Collections, TCPTypes, CardTypes, GinTypes, GinClasses,
...
type
	...
	protected
		procedure OnSocketConnected(ASender: TObject);
		procedure OnSocketDisconnected(ASender: TObject);
	...
	public
		...
		function  PrepareRead(var ABuf: TMsgData): Integer;
	...
	public
		...
		Reflector: TMessageReflector;

		constructor Create;
		destructor  Destroy; override;

		procedure SendApplicationMessage(const AMessage: Cardinal;
				const AWParam: IntPtr; const ALParam: IntPtr);
...
//				AText: AnsiString);
		...
		function ReadConnectionData(AConnection: TTCPConnection;
				ABuf: TMsgData; ASize: Integer): Boolean;
...
{$ENDIF}
			...
			if  CompareText(string(AMessageList.Locale),
					string(Client.Room)) = 0 then
				Client.SendApplicationMessage(YCM_UPDATEROOMUSERS, IntPtr(@data), 1);
			end
		else
			Client.SendApplicationMessage(YCM_UPDATEROOMLIST, IntPtr(@AData), 1)
	else if  CompareText(string(AMessageList.Desc), ARR_LIT_NAM_CATEGORY[mcPlay]) = 0 then
{$IFDEF ANDROID}
		if  AnsiLength(AMessageList.Locale) > 0 then
{$ELSE}
...
{$ENDIF}
			begin
			p:= Pos(' ', data);
			u:= Copy(data, Low(data), p);
			d:= Copy(data, p + 1, MaxInt);

			s:= Ord(d[Low(string)]) - $30;

			if  Assigned(Client.Game) then
				begin
				Client.Game.Slots[s].Name:= AnsiString(u);
				Client.SendApplicationMessage(YCM_UPDATESLOTSTATE,
						IntPtr(Client.Game.State), s);
				end;
			end
		else
			Client.SendApplicationMessage(YCM_UPDATEGAMELIST, IntPtr(@data), 1);
	end;
...
function TTCPConnection.PrepareRead(var ABuf: TMsgData): Integer;
	begin
	try
		if  Connected then
			begin
			Socket.ReadTimeout:= 1;
			Socket.IOHandler.ReadBytes(TIdBytes(ABuf), -1, True);
...
function TTCPConnection.ProcessSendMessages: Boolean;
	...
	while SendMessages.Count > 0 do
		...
		try
			Socket.IOHandler.Write(TIdBytes(buf), Length(buf));
...
procedure TTCPConnection.OnSocketConnected(ASender: TObject);
	begin
...
procedure TTCPConnection.OnSocketDisconnected(ASender: TObject);
	begin
//dengland THIS IS F***ING NOT BEING CALLED!!!  I HATE F****ING INDY COMPONENTS!
	...
	if  AMessage.Method = $01 then
		...
		if  AMessage.Params.Count > 1 then
			...
			if  (CompareText(string(AMessage.Params[0]), string(Room)) = 0) then
				...
				SendApplicationMessage(YCM_UPDATEROOMUSERS, IntPtr(@s), 1);
				end;
		...
		if  AMessage.Params.Count > 1 then
			...
			if  CompareText(string(AMessage.Params[1]), string(OurIdent)) = 0 then
				...
				begin
				s:= string(AMessage.Params[1]);
				SendApplicationMessage(YCM_UPDATEROOMUSERS, IntPtr(@s), 2);
				end;
...
procedure TGinClient.ProcessConnectMessage(AConnection: TTCPConnection;
	...
	if  AMessage.Method = $00 then
		begin
		HostLogMessages.Add('');
		HostLogMessages.Add('!!Connection Error:  ' + string(AMessage.DataToString));
		end
...
{$IFDEF ANDROID}
				Game.OurSlot:= Ord(Char(AMessage.Params[2][1])) - $30;
{$ELSE}
...
{$ENDIF}
			...
			if  Assigned(Game) then
				if  (CompareText(string(AMessage.Params[0]),
					...
					begin
{$IFDEF ANDROID}
					i:= Ord(Char(AMessage.Params[2][1])) - $30;
{$ELSE}
...
{$ENDIF}
		...
		if  AMessage.Params.Count > 1 then
			...
			if  Assigned(Game) then
				...
				else if (CompareText(string(AMessage.Params[0]), string(Game.Ident)) = 0) then
					begin
			...
			if  Assigned(Game) then
				...
				for i:= 0 to High(Game.Slots) do
					SendApplicationMessage(
							YCM_UPDATESLOTSTATE, Ord(Game.State), i);
		...
		if  Game.Slots[i].State = psPreparing then
			...
			if  i = Game.OurSlot then
				...
				SendApplicationMessage(YCM_UPDATEDRAWCARD, AMessage.Data[1],
						AMessage.Data[2]);
				end
			else
		...
		if  Length(AMessage.Data) = 2 then
			begin
			SendApplicationMessage(YCM_UPDATEDISCARD, AMessage.Data[0], AMessage.Data[1]);
...
procedure TGinClient.SendApplicationMessage(const AMessage: Cardinal;
		const AWParam: IntPtr; const ALParam: IntPtr);
	begin
	Reflector.SendMessage(AMessage, AWParam, ALParam);
	end;
...
{$IFDEF ANDROID}
	if  AnsiLength(APassword) > 0 then
{$ELSE}
...
//				SetLength(buf, j);
			...
			i:= Length(FInputBuf);
			SetLength(FInputBuf, i + Length(ABuf));

			Move(ABuf[0], FInputBuf[i], Length(ABuf));
